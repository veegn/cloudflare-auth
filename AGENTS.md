...
- Schema changes: `schema.sql` uses `CREATE TABLE IF NOT EXISTS`. New columns on **existing** D1 need explicit `ALTER TABLE` via `wrangler d1 execute --remote` (scripts do not migrate live DBs). Migrations: `migrations/001_apps_icon_url.sql` (`apps.icon_url`); `migrations/002_media_r2.sql` (`users.avatar_object_key`, `apps.icon_object_key`).
...
| `src/media.rs` | Mandatory server-side crop → resize → JPEG for uploads |
| `src/media_r2.rs` | R2 `MEDIA` binding helpers (avatars / app icons) |
...
| `public/i18n.js` | en/zh dictionaries + sidebar language switch (`localStorage.cfa_lang`) |
...
- Console i18n: HTML uses `data-i18n*` attrs; `console.js` uses `I18N.t()`; load order `i18n.js` then `console.js`. Project docs: `README.md` (zh) + `README.en.md` (en).
...
- R2 media: bucket `cloudflare-auth-media`, binding **`MEDIA`**. Uploads are **raw image bodies** (`POST /auth/avatar`, `POST /apps/:id/icon`). Worker always center-crops, resizes to 256px, and JPEG-compresses (`src/media.rs`) before R2 put. Keys: `avatars/{user_id}.jpg`, `apps/{app_id}.jpg`. Serve path prefers R2 → external icon_url → identicon.
