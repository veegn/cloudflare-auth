...
async fn route(
    ...
    // 第三方：用户基础信息（id / email / 头像）
    if method == Method::Get && path == "/userinfo" {
        return handle_userinfo(req, env).await;
    }
    if method == Method::Get && path == "/v1/userinfo" {
        return handle_userinfo(req, env).await;
    }
    ...
    if method == Method::Get {
        if let Some(rest) = path.strip_prefix("/users/") {
            ...
            // 第三方服务端：用 App 凭证按 userId 查基础信息
            if let Some(uid) = rest.strip_suffix("/profile") {
                return handle_user_profile_by_id(req, env, uid).await;
            }
        }
...
async fn handle_me(req: &Request, env: &Env) -> ApiResult<Response> {
    let (user, _, app_id) = require_auth(req, env).await?;
    ok_json(json!({ "user": public_user(&user), "appId": app_id }))
}

/// 第三方展示用：用户 ID / 邮箱 / 头像（绝对 URL）
/// 鉴权：Bearer 用户 token；若带 X-App-Id/Secret 则校验会话属于该 App。
async fn handle_userinfo(req: &Request, env: &Env) -> ApiResult<Response> {
    let (user, _, session_app_id) = require_auth(req, env).await?;
    ...
    if app_id.is_some() || app_secret.is_some() {
        let app = resolve_app_credentials(env, app_id.as_deref(), app_secret.as_deref()).await?;
        match app {
            Some(app) => {
                // 会话必须绑定到该 App（重定向登录 / 带凭证登录时写入）
                if session_app_id.as_deref() != Some(app.app_id.as_str()) {
                    return Err(api_err(
                        403,
                        "token_app_mismatch",
...
async fn handle_user_profile_by_id(
    req: &Request,
    env: &Env,
    user_id: &str,
) -> ApiResult<Response> {
...
fn userinfo_payload(env: &Env, user: &crate::util::UserRow) -> Value {
